#include<iostream>
using namespace std;
int GetDigit(int date[], int n){//此函数用于求解所给数组中最大数有几位
	int _digit = 0;
	for (int i = 0; i<n; i++){
		...
		if (count>_digit)
			_digit = count;//把最大位数赋值_digit
	}
	return _digit;
}
void basesort(int date[], int n){
	int _digit = GetDigit(date, n);//取得最大位数
	int base = 1;
	for (int i = 0; i < _digit; i++){//位数决定排序循环次数
		...
		for (int j = 0; j < n; j++){
			int fg = date[j] / base;//base是变量，因为每次循环比较的位是不同的
			int k = fg % 10;
			count[k]++;
			int te = count[k] - 1;
